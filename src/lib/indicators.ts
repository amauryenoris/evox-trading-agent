...
export function calculateAllIndicators(bars: AlpacaBar[]): TechnicalIndicators {
  ...
  return {
    ...
    closeMinus2: bars.length >= 3 ? bars[bars.length - 3].c : null,
    closeMinus3: bars.length >= 4 ? bars[bars.length - 4].c : null,
    closeMinus4: bars.length >= 5 ? bars[bars.length - 5].c : null,

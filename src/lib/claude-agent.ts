    ...
    try {
      ...
      if (!setup_detected) {
        ...
        decisions.push({
          ...
          timestamp,
          symbol,
          decision: { action: 'HOLD', symbol, quantity: 0, reasoning: `Setup gate: no mean reversion setup (z-score ${zScore.toFixed(3)} > ${effectiveThreshold.toFixed(2)}) and no trend setup (price not above EMA50)`, confidence: 0 },
          indicators,
          portfolioSnapshot: { equity: account.equity, cash: account.cash, positionCount: positions.length },
          orderExecuted: false,
          error: undefined,
